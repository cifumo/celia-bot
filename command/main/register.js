...
export default {
    ...
    example: "Example: %p%cmd cifumo 19",
    name: "daftar",
    tags: "main",

    run: async(m, { conn, text }) => {
        let number = /[0-9]{1,3}/i, user = db.data.users[m.sender]
        let data = text.match(number), equality = Object.values(db.data.users).map(v => v.name)

        if (!data[0]) return m.reply(`Example: ${m.prefix + m.command} celia 16`)
